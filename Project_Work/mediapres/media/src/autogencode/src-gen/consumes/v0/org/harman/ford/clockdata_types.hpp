...
struct clockdata_types {
    ...
    struct GlobalClock_Data_HS3_struct : CommonAPI::Struct<uint8_t, uint16_t, uint8_t, uint8_t, uint8_t, uint8_t> {
    	
    	GlobalClock_Data_HS3_struct() {
    	}
    	GlobalClock_Data_HS3_struct(const uint8_t &_GlblClkYr_No_Actl, const uint16_t &_GlblClkDay_No_Actl, const uint8_t &_GlblClkMnte_No_Actl, const uint8_t &_GlblClkScnd_No_Actl, const uint8_t &_GlblClkHr_No_Actl, const uint8_t &_status)
    	{
    	...
    	inline const uint8_t &getStatus() const { return std::get<5>(values_); }
    	inline void setStatus(const uint8_t &_value) { std::get<5>(values_) = _value; }
    	inline bool operator==(const GlobalClock_Data_HS3_struct& _other) const {
                return (getGlblClkYr_No_Actl() == _other.getGlblClkYr_No_Actl() && getGlblClkDay_No_Actl() == _other.getGlblClkDay_No_Actl() && getGlblClkMnte_No_Actl() == _other.getGlblClkMnte_No_Actl() && getGlblClkScnd_No_Actl() == _other.getGlblClkScnd_No_Actl() && getGlblClkHr_No_Actl() == _other.getGlblClkHr_No_Actl() && getStatus() == _other.getStatus());
        }
    	inline bool operator!=(const GlobalClock_Data_HS3_struct &_other) const {
    		return !((*this) == _other);
    	}

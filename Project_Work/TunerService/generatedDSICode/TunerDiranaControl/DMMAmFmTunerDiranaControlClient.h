#ifndef DMMAMFMTUNERDIRANACONTROLCLIENT_H
#define DMMAMFMTUNERDIRANACONTROLCLIENT_H
/**********************************************************************
 *  Project       Harman Car Multimedia System
 *  (c) copyright 2014
 *  Company       Harman/Becker Automotive Systems GmbH
 *                All rights reserved
 **********************************************************************/

/*
 * Generated by DSI Generator version 2.0
 * Date: 21.11.2014
 */


#include "DMMAmFmTunerDiranaControl.h"


DSI_EXTERN_C_BEGIN


/*
 * Only the DSI framework is allowed to modify the attributes.
 * For the user of the proxy the attributes are constant!
 */
#ifndef DSICONST
#ifdef __cplusplus
   #define DSICONST
#else
   #define DSICONST const
#endif
#endif


/**
 * Error indication for synchronous requst calls.
 */
typedef struct _DMMAmFmTunerDiranaControlErrorIndicator
{
   /// either request or response identifier or 0 on OS error.
   DSICONST int id;
} DMMAmFmTunerDiranaControlErrorIndicator;


/* -------------------------------------------------------------------------------
 *  synchronous method calls with corresponding response (containing data)
 * ------------------------------------------------------------------------------- */


/* -------------------------------------------------------------------------------
 *  synchronous attribute fetch
 * ------------------------------------------------------------------------------- */



DSI_EXTERN_C_END


#endif // DMMAMFMTUNERDIRANACONTROLCLIENT_H

...
3.run cmake with SYSROOT as command line argument and finally run make(with one of available options) to build the default implementation that is generated.

***command :***

		//to generate code
			export JAVA_HOME=<JAVA_PATH>
			//One can set the DCIF_CODE_GENERATOR and CMAKE_SYSROOT via environment variable or pass it as a commandline argument for cmake
			export DCIF_CODE_GENERATOR=<PATH_TO_DCIF_CODE_GEN_JAR>
			export CMAKE_SYSROOT=<PATH_TO_SYS_ROOT>
			// DCIF_CODE_GENERATOR path till .jar
			cmake -DDCIF_CODE_GENERATOR=<PATH_TO_DCIF_CODE_GEN_JAR> -DCDL_DIR=<PATH_TO_CDL> -DCMAKE_SYSROOT=<PATH_TO_SYS_ROOT> <CMAKELIST_PATH>
			make generate

		//to compile code
			cmake -DCMAKE_SYSROOT=<PATH_TO_SYS_ROOT> <CMAKELIST_PATH>
			make compile
		 
		//to clean the compiled code
			make clean
		 
		//to clean the generated files and compiled code
			make cleanall

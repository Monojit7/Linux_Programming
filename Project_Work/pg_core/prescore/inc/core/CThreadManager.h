...
private:
	...
	public:
		...
		ThreadData(ThreadData&& data) :
			mThread(std::move(data.mThread)),
			mDataHandler(data.mDataHandler)
		{
...
public:
	...
	static CThreadManager* self();

	void invokeDataHandler(int threadId,BaseData* data = nullptr);

...
public:
   ...
   bool startThread(int& threadId,void (*threadFunc)(IThreadDataHandler::SendDataFunction &,ts...),ts... arg )
   {
	   int id = -1;
	   bool status = false;
	   LOG_DEBUG(Log::e_LOG_CONTEXT_CORE, "CThreadInterface::startThread - argument list size=%d", sizeof...(arg));
	   id = getThreadManager()->startThread(threadId, mDataHandler, threadFunc, arg...);
	   if (id > 0)
	   {
	   ...
	   LOG_DEBUG(Log::e_LOG_CONTEXT_CORE, "CThreadInterface::createThread - threadIDList size=%d\n", mThreadIdList.size());
	   return status;
   }
	...
	// returns the number of active threads in the givn threadInterface object.
	int getSize();
private:
	std::list<int> mThreadIdList;
	static CThreadManager* getThreadManager();
	void setThreadManager();
	static CThreadManager* mThreadManager;
